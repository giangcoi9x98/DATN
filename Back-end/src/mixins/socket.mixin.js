...
module.exports = {
	...
	started() {
		...
		// 	// 		this.logger.debug("add authorize middleware");
		...
		call: {
			...
			async handler(ctx) {
				...
				if (!_.isString(action)) {
					this.logger.debug(
						"BadRequest:action is not string! action:",
						action
					);
				...
				if (handlerItem.onAfterCall) {
					res =
						(await handlerItem.onAfterCall.call(
							this,
							ctx,
							socket,
							res
	...
	methods: {
		initSocketIO(srv, opts) {
			...
			// console.log("aaaaa",srv);
			if (
				this.settings.cors &&
				this.settings.cors.origin &&
				!opts.origins
			) {
				// cors settings
				opts.origins = function (origin, callback) {
					if (
						...
						this.logger.debug(
							`origin ${origin} is allowed by checkOrigin.`
						);
						callback(null, true);
					} else {
...
function makeAuthorizeMiddleware(svc, handlerItem) {
	return async function authorizeMiddleware(socket, next) {
		try {
			...
			console.log("makeAuthorizeMiddleware ERROR", e);
			return next(e);
		}
	};
}
...
function makeHandler(svc, handlerItem) {
	...
	return async function (action, params, respond) {
		...
		try {
			...
			let res = await svc.actions.call({
				socket: this,
				action,
				params,
				handlerItem,

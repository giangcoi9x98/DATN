...
console.log(`path.resolve(__dirname, '../.env')`,path.resolve(__dirname, '../../env-file'));
...
function require_actions(actions) {
    const actions_list = actions.map(action_path => require(`../actions/${action_path}.action.js`))
    let actions_obj = {}
...
function calculateNextCouponPaymentTimestamp(data) {
    ...
    if (data.coupon_payment_period === PaymentPeriod.End) {
        ...
        next_coupon_payment_timestamp = moment.utc().add(data.term, data.term_by);
    } else if (data.coupon_payment_period === PaymentPeriod.Semiannual) {
        next_coupon_payment_timestamp = moment.utc().add(6, 'M').valueOf()
    } else {
        next_coupon_payment_timestamp = moment.utc().add(1, TimeShorthand[data.coupon_payment_period]).valueOf()
    }
...
async function checkIsAuthenticated(ctx, redis) {
    ...
    const publicKey = Buffer.from(process.env.AUTH_PUBLIC_KEY || '', 'base64').toString('utf-8');
    const {
        token
    } = ctx.meta;
    ...
    try {
        ...
        if (ctx.action.require_2fa_if_enable && this) {
            const isUserEnable2FA = await this.broker.call('user.getIsUserEnable2FA', {
                user_id: decoded.id
            })
            if (isUserEnable2FA) {
    ...
    const redisToken = await redis.get(`${accessTokenPrefix}${decoded.loginId}`);
    if (token !== redisToken) {
        throw new MoleculerError('Invalid Token', 401, Langs.TOKEN_INVALID);
    }
    const bannedTokenTime = Number(await redis.get(`${bannedTokenPrefix}${decoded.id}`));

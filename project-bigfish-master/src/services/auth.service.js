		...
		register: {
			...
			async handler(ctx) {
				...
				const isExists = await this.mysql.queryOne(
					"SELECT COUNT(accountId) as isExists FROM Account WHERE email = ?",
					[email]
				);
				...
				try {
					...
					const accountInfoId = uuidv1;
					await this.mysql.query(
						`INSERT INTO Account (accountId,email,password,accountInfoId)
            VALUES (?,?,?,?)  `,
						[
							accountId,
              ...
              VALUES (?, ?, ?, ?, ?) `,
						[
							...
							deviceName,
							newUserData.accountId,
						]
					);
					const loginRecord = this.mysql.query(
